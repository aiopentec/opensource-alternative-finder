...
CATEGORY_CONTENT_CACHE_PATH = Path(__file__).parent.parent / "data" / "cache" / "category_content.json"


def load_category_content_cache() -> dict:
    """Expanded category-hub-page content (intro + FAQ) generated by
    category_content_generator.py, keyed by category slug (e.g. 'communication').
    Returns {} if the cache doesn't exist yet, in which case the category page
    falls back to the original bare link-list with no intro/FAQ."""
    if CATEGORY_CONTENT_CACHE_PATH.exists():
        with open(CATEGORY_CONTENT_CACHE_PATH) as f:
            return json.load(f)
    return {}


CATEGORY_CONTENT = load_category_content_cache()
...
def build_site(cache_dir: str = '.cache/publish', site_dir: str = 'site'):
  ...
  <div class="card" data-category="{category}" data-search="{search_data}">
    ...
    for category in categories:
        ...
        cat_label = category.replace('-', ' ').title()
        cat_icon = CATEGORY_ICONS.get(category, '🔧')
        cat_url = f"{SITE_BASE_URL}/{category}/"

        cat_cards = '\n'.join(
            f'''<li class="cat-item">
      <a href="../{c["slug"]}/" class="cat-item-title">{c["title"]}</a>
      <span class="cat-item-price">{c.get("proprietary_pricing","N/A")} → <strong style="color:var(--green)">{c.get("oss_pricing","Free")}</strong></span>
    </li>'''
            for c in cat_comps
        )

        cat_content = CATEGORY_CONTENT.get(category, {})
        cat_intro = cat_content.get('intro', '')
        cat_faq = cat_content.get('faq', [])

        intro_html = f'<p class="cat-intro">{cat_intro}</p>' if cat_intro else ''
        ...
        if cat_faq:
            ...
            faq_html = f'<div class="cat-faq"><h2>Frequently Asked Questions</h2>{faq_items_html}</div>'
            faq_entities = ',\n'.join(
                f'{{"@type":"Question","name":{json.dumps(item.get("q",""))},"acceptedAnswer":{{"@type":"Answer","text":{json.dumps(item.get("a",""))}}}}}'
                for item in cat_faq
            )
            faq_schema = f''',
    {{
      ...
      "mainEntity": [{faq_entities}]
        ...
        cat_meta_desc = (
            (cat_intro[:157].rsplit(' ', 1)[0] + '…') if cat_intro
            else f"Free open-source alternatives to popular {category.replace('-',' ')} tools. Detailed comparisons with pricing and migration guides."
        )
...
<title>{cat_label} Open Source Alternatives | OS Alternative Finder</title>
...
<meta property="og:type" content="website">
<meta property="og:title" content="{cat_label} Open Source Alternatives">
<meta property="og:description" content="{cat_meta_desc}">
...
<meta name="twitter:card" content="summary_large_image">
<meta name="twitter:title" content="{cat_label} Open Source Alternatives">
<meta name="twitter:description" content="{cat_meta_desc}">
<meta name="twitter:image" content="{SITE_BASE_URL}/og-default.png">
...
{{
  ...
  "@graph": [
    {{
      "@type": "Article",
      "headline": "{cat_label} Open Source Alternatives",
      "description": {json.dumps(cat_meta_desc)},
      "image": "{SITE_BASE_URL}/og-default.png",
      "dateModified": "{datetime.utcnow().strftime('%Y-%m-%d')}",
      "author": {{"@type": "Organization", "name": "Open Source Alternative Finder", "url": "{SITE_BASE_URL}"}},
      "publisher": {{
        "@type": "Organization",
        "name": "Open Source Alternative Finder",
        "url": "{SITE_BASE_URL}",
        "logo": {{"@type": "ImageObject", "url": "{SITE_BASE_URL}/logo-square.png"}}
      }}
    }},
    {{
      "@type": "BreadcrumbList",
...
<style>
:root {{--blue:#1F5C99;--blue-light:#2980B9;--green:#1A7A3F;--green-bg:#EAFAF1;--bg:#F0F4F8;--card:#fff;--border:#E2E8F0;--text:#1A202C;--text-muted:#718096;}}
body{{font-family:system-ui,sans-serif;max-width:840px;margin:0 auto;padding:2rem 1rem;background:var(--bg);color:var(--text);line-height:1.6;}}
a{{color:var(--blue);}}
h1{{font-size:1.8rem;margin-bottom:0.25rem;}}
.cat-intro{{background:var(--card);border:1px solid var(--border);border-radius:10px;padding:1.25rem 1.5rem;margin:1.25rem 0;color:var(--text);}}
.cat-list{{list-style:none;margin:1.5rem 0;padding:0;display:flex;flex-direction:column;gap:0.6rem;}}
.cat-item{{display:flex;justify-content:space-between;align-items:center;gap:1rem;background:var(--card);border:1px solid var(--border);border-radius:8px;padding:0.85rem 1.1rem;}}
.cat-item-title{{font-weight:600;text-decoration:none;}}
.cat-item-price{{font-size:0.85rem;color:var(--text-muted);white-space:nowrap;}}
.cat-faq{{margin:2rem 0;}}
.cat-faq h2{{font-size:1.25rem;}}
.faq-item{{margin:1rem 0;}}
.faq-item h3{{font-size:1rem;margin-bottom:0.25rem;}}
.faq-item p{{color:var(--text-muted);margin:0;}}
footer{{margin-top:3rem;color:var(--text-muted);font-size:0.85rem;border-top:1px solid var(--border);padding-top:1rem;}}
</style></head>
<body>
<h1>{cat_icon} {cat_label} Comparisons</h1>
<p><a href="../">← All categories</a></p>
{intro_html}
<ul class="cat-list">{cat_cards}</ul>
{faq_html}
<footer>Open Source Alternative Finder · Updated {updated} · <a href="../privacy/">Privacy Policy</a></footer>
</body></html>"""
