...
def build_savings_calculator(site_dir: str):
    """Copy savings calculator from repo root to site output."""
    calc_dir = Path(site_dir) / 'savings-calculator'
    calc_dir.mkdir(exist_ok=True)
    src = Path(__file__).parent.parent / 'savings_calculator.html'
    if src.exists():
        import shutil
        shutil.copy(src, calc_dir / 'index.html')
        logger.info("   💰 savings-calculator/index.html")
    else:
        logger.warning("   ⚠️  savings_calculator.html not found in repo root")
